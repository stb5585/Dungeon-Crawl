...
class BasePopupMenu:
    ...
    def draw_popup(self, player_char):
        ...
        # Title
        title_text = self.title_font.render(self.title, True, self.YELLOW)
    ...
    def draw_list(self):
        ...
        for idx in range(start, end):
            ...
            if idx == self.selected_index:
                ...
                text = self.normal_font.render(text_str, True, self.YELLOW)
            else:
    ...
    def draw_details(self, player_char):
        ...
        attrs = []
        for key in ("desc", "description", "slot", "type", "value", "weight"):
            if hasattr(item, key):
        ...
        # Custom details hook
        self.draw_details_extra(player_char, item, x, y)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if self.items:
                            result = self.on_select(player_char, self.items[self.selected_index])
                        ...
                        self.scroll_offset = min(max(0, len(self.items) - max_visible), self.scroll_offset + max_visible)
...
class InventoryPopupMenu(BasePopupMenu):
    ...
    def build_items(self, player_char):
        items = []
        # Flatten inventory dict: {category: [items]}
    ...
    def draw_details(self, player_char):
        """Override to extract object from (category, obj) tuple."""
        if not self.items:
            x = self.details_rect.left + 16
            y = self.details_rect.top + 12
            self.screen.blit(self.normal_font.render("No items", True, self.GRAY), (x, y))
        ...
        category, obj = self.items[self.selected_index]
        x = self.details_rect.left + 16
        y = self.details_rect.top + 12
        ...
        # Custom details hook
        self.draw_details_extra(player_char, (category, obj), x, y)
    ...
    def on_select(self, player_char, item):
        ...
        actions = []
        item_slot = getattr(obj, "slot", None)
        
        # Can equip if it has a slot and isn't already equipped
        if item_slot and item_slot in ("Weapon", "Armor", "OffHand", "Ring", "Pendant"):
            current_equipped = player_char.equipment.get(item_slot)
            if current_equipped != obj:
                actions.append("Equip")
        
        # Check if item is usable (consumable)
        if category in ("Consumable", "Potion", "consumable", "potion"):
            actions.append("Use")
    ...
    def _equip_item(self, player_char, item, category):
        ...
        if current:
            # Add to inventory
            inv_category = getattr(current, "type", "Misc")
            if inv_category not in player_char.inventory:
                player_char.inventory[inv_category] = []
            player_char.inventory[inv_category].append(current)
    ...
    def _use_item(self, player_char, item, category):
        ...
        # Apply item effects (simplified - extend based on your item system)
        if hasattr(item, "heal"):
            heal_amount = item.heal
            player_char.health.current = min(player_char.health.max, player_char.health.current + heal_amount)
        
        if hasattr(item, "mana_restore"):
            mana_amount = item.mana_restore
            player_char.mana.current = min(player_char.mana.max, player_char.mana.current + mana_amount)
        
        # Remove item from inventory
        if category in player_char.inventory and item in player_char.inventory[category]:
            # Check for quantity
            if hasattr(item, "qty") and item.qty > 1:
                item.qty -= 1
            else:
                player_char.inventory[category].remove(item)
                if not player_char.inventory[category]:
...
class EquipmentPopupMenu(BasePopupMenu):
    ...
    def item_display_text(self, item):
        slot, obj = item
        name = getattr(obj, "name", "(empty)") if obj else "(empty)"
    ...
    def on_select(self, player_char, item):
        ...
        # Can't act on empty slots
        if obj is None:
            return None
        
        # Show action menu
        actions = ["Unequip", "Cancel"]
        action_popup = SelectionPopup(
            self.presenter,
            self.parent_screen,
            title=f"Action: {getattr(obj, 'name', 'Item')}",
    ...
    def _unequip_item(self, player_char, slot, item):
        ...
        # Add to inventory
        category = getattr(item, "type", "Misc")
        if category not in player_char.inventory:
            player_char.inventory[category] = []
        player_char.inventory[category].append(item)
...
class QuestPopupMenu(BasePopupMenu):
    ...
    def draw_details(self, player_char):
        ...
        name_text = self.large_font.render(quest_name, True, self.YELLOW)
        self.screen.blit(name_text, (x, y))
        y += name_text.get_height() + 8
    ...
    def draw_details_extra(self, player_char, item, x, y):
        """Not used - draw_details is overridden."""
        pass
    ...
    def _draw_quest_details(self, quest_data, x, y):
        ...
        if turned_in:
            ...
            status = "In Progress"
        text = self.normal_font.render(f"Status: {status}", True, self.YELLOW if completed and not turned_in else self.WHITE)
        self.screen.blit(text, (x, y))
        y += self.line_height * 1.5
        
        # Rewards
        text = self.normal_font.render("Rewards:", True, self.YELLOW)
        self.screen.blit(text, (x, y))
        y += self.line_height
...
class SimpleListPopupMenu(BasePopupMenu):
    ...
    def build_items(self, player_char):
        ...
        if callable(raw):
            raw = raw()
        self.items = list(raw) if raw else []
        self.selected_index = 0
        self.scroll_offset = 0
    ...
    def draw_details_extra(self, player_char, item, x, y):
        # For strings, just re-render larger or show placeholder
        if isinstance(item, str):
            text = self.normal_font.render(item, True, self.WHITE)
            self.screen.blit(text, (x, y))
