...
| Progression pacing | Beast Master companion to `Trusted` | Manual playtest | Raise one active companion to at least bond 25. | `Pending Manual` | Record companion name/species, combat count, actions, victories, Favored Enemy state, ring state, replacement/tame interruptions, and cadence feel. | Mark `Watch` if `Trusted` is not near 5-6 ordinary active wins or command output dominates direct turns. |
