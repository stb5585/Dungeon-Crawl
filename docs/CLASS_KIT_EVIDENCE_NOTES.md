...
| Progression pacing | Thaumaturgist Xenid bond to `50` | Manual playtest | Raise one Xenid bond to at least 50 without switching Xenids. | `Pending Manual` | Record Xenid name, combat count, action count, victories, recall/death interruptions, ring state, and cadence feel. | Mark `Watch` if focused bond progress misses expected cadence or switching pressure feels unclear. |
