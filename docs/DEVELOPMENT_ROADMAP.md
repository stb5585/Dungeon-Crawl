...
1. Complete modern Character Menu acceptance and iteration.
   ...
   - Implemented: generic Character/Equipment tabs, wider Character panel with half-width right-column level-progress bar, larger portrait placeholder, bottom-anchored Core Attributes, larger character/combat/resistance text, right-aligned combat values, adaptive side-by-side weaknesses/resistances for all 10 resistance keys, panel dividers, spread-out paper-doll Equipment tab with Helmet UI placeholder, equipment-buff reporting, and town/dungeon opt-in routing.
