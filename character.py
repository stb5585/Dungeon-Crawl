...
def new_char() -> object:
    ...
    player = Player(location=[location_x, location_y, location_z], state='normal', cls=cls.name,
                    ...
                    charisma=stats[4], dex=stats[5], pro_level=1, gold=stats[4] * 25,
                    status_effects={"Stun": [False, 0], "Poison": [False, 0, 0], "DOT": [False, 0, 0],
                                    "Doom": [False, 0]}, equipment=cls.equipment, inventory={},
...
def load(char=None) -> dict:
    ...
    """
    if char is None:
        load_list = glob.glob("save_files/*.save")
        choice = display.load_menu(load_list)
        if not choice:
            return False, False
        save_file = load_list[choice]
...
class Player(Character):
    ...
    def __init__(self, location, state, cls, level, exp, health, health_max, mana, mana_max, strength, intel,
        ...
        self.state = state  # currently 'normal' or 'fight'
        self.cls = cls
        self.level = level
        self.experience = exp
        self.health = health
    ...
    def get_event(self, event):
        # add directional images for up, down, left, right (may need to mirror right for left)
    ...
    def draw(self, surf, menu=False, i=0):
        if menu:
            i //= 20
            walk_images = glob.glob('resources/player/'+self.cls.lower()+'/Walk/*')
            walk_image = pygame.image.load(walk_images[i])
            image = pygame.transform.scale(walk_image, (200, 200))
            surf.blit(image, (20, -15))
        else:
    ...
    def cast_spell(self, enemy, ability):
        ...
        if ability().cat == 'Attack':
            ...
            if random.randint(0, enemy.dex // 2) > \
                    random.randint((self.intel + spell_mod) // 2, self.intel + spell_mod) and \
                    not enemy.status_effects['Stun'][0]:
    ...
    def status(self):
        ...
        display.status_menu(self, status_list, equip_list)
    ...
    def save(self):
        ...
        save_name = now.strftime("%Y_%m_%d_%H_%M") + self.name + "_" + self.cls + "_" + str(self.level)
        while True:
            save_file = "save_files/%s.save" % save_name
