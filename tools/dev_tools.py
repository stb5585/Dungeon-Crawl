...
def test_event_system(args):
    ...
    bus.emit_simple(EventType.COMBAT_START, data={"player": "TestPlayer", "enemy": "TestEnemy"})

    bus.emit_simple(
        EventType.DAMAGE_DEALT, data={"actor": "TestPlayer", "target": "TestEnemy", "damage": 25}
    )

    bus.emit_simple(EventType.COMBAT_END, data={"winner": "TestPlayer"})
...
def test_action_queue(args):
    ...
    queue.schedule(
        char1, ActionType.ATTACK, mock_action, target=char2, priority=ActionPriority.HIGH
    )

    queue.schedule(
        char2, ActionType.SPELL, mock_action, target=char1, priority=ActionPriority.NORMAL, delay=1
    )

    queue.schedule(
        char1, ActionType.SKILL, mock_action, target=char2, priority=ActionPriority.NORMAL
    )
    ...
    for i, action in enumerate(queue.queue):
        print(
            f"  {i+1}. {action.actor.name}: {action.action_type.value} "
...
Examples:
  ...
  python tools/dev_tools.py generate --output src/core/data/abilities
    ...
    balance_parser.add_argument(
        "--iterations", "-i", type=int, default=100, help="Number of simulations"
    )
