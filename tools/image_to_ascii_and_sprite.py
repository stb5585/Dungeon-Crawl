...
def image_to_sprite(img: Image.Image, size: int, colors: int, dither: bool) -> Image.Image:
    ...
    quantized = resized.convert("RGBA").quantize(
        colors=colors, method=Image.Quantize.MEDIANCUT, dither=dither_mode
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--name", type=str, default=None, help="Output base name (without extension)"
    )
    ...
    parser.add_argument(
        "--colors", type=int, default=32, help="Number of colors for sprite quantization"
    )
    parser.add_argument("--dither", action="store_true", help="Enable dithering for sprite")
    parser.add_argument(
        "--ascii-out", type=str, default="ascii_files", help="ASCII output directory"
    )
    parser.add_argument(
        "--sprite-out",
        type=str,
        default="src/ui_pygame/assets/sprites",
    ...
    with Image.open(image_path) as img:
        ...
        sprite_img = image_to_sprite(
            img, size=args.sprite_size, colors=args.colors, dither=args.dither
        )
