...
class PlayerPresentationMixin:
    ...
    def _class_kit_status_str(self):
        ...
        if cls_name in {"Sorcerer", "Wizard"}:
            ...
            values = " ".join(
                f"{school[:3]}:{affinity[school]:.1f}" for school in wizard.AFFINITY_SCHOOLS
            )
            lines.append(f"{'Affinity:':13} {values} /{cap}")
        if cls_name == "Lycan":
            state = lycan.ensure_state(self)
            lines.append(
                f"{'Moon:':13} {state['moon_phase']} ({state['moon_steps']}/{lycan.STEPS_PER_PHASE})"
            )
        ...
        if cls_name == "Soulcatcher":
            harvested = class_rings.ensure_state(self)["data"]["Soulcatcher"].get(
                "harvested_types", []
            )
    ...
    def combat_str(self):
        ...
        main_crit = int(
            (self.equipment["Weapon"].crit + (BASE_CRIT_PER_POINT * self.check_mod("speed"))) * 100
        )
        if self.equipment["OffHand"].typ == "Weapon":
            off_dmg = self.check_mod("offhand")
            off_crit = int(
                (self.equipment["OffHand"].crit + (BASE_CRIT_PER_POINT * self.check_mod("speed")))
                * 100
            )
            combat_message += f"{'Attack:':16}{' ':2}{str(main_dmg):>3}/{str(off_dmg):>3}\n"
            combat_message += (
                f"{'Critical Chance:':16}{' ':2}{str(main_crit):2}%/{str(off_crit):>2}%\n"
            )
