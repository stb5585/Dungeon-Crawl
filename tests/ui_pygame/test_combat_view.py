...
def test_combat_log_wraps_long_charge_messages(monkeypatch):
    ...
    assert view._wrapped_combat_log_lines(view.combat_width - 30, font=font) == ["Goblin is charging."]
...
def test_telegraph_banner_uses_latest_matching_log_line(monkeypatch):
    view = _make_view()
    fonts = [RecordingFont(), RecordingFont(), RecordingFont(), RecordingFont(), RecordingFont(), RecordingFont()]
    rect_calls = []
...
def test_counted_art_status_icons_render_badge_over_icon(monkeypatch):
    ...
    blitted_text = [getattr(surface, "text", "") for surface, _pos, _args, _kwargs in view.screen.blit_calls]
