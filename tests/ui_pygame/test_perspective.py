...
def test_perspective_transform():
    ...
    try:
        ...
        left_quad = (
            0,
            0,  # top-left (unchanged)
            wall_width,
            inset,  # top-right (move down/in)
            wall_width,
            wall_height - inset,  # bottom-right (move up/in)
            0,
            wall_height,  # bottom-left (unchanged)
        )

        left_wall = scaled.transform(
            (wall_width, wall_height), Image.QUAD, left_quad, Image.BICUBIC
        )
        ...
        right_quad = (
            0,
            inset,  # top-left (move down/in)
            wall_width,
            0,  # top-right (unchanged)
            wall_width,
            wall_height,  # bottom-right (unchanged)
            0,
            wall_height - inset,  # bottom-left (move up/in)
        )

        right_wall = scaled.transform(
            (wall_width, wall_height), Image.QUAD, right_quad, Image.BICUBIC
        )
        ...
        floor_quad = (
            ...
            0,  # top-right (narrow)
            floor_width,
            floor_height,  # bottom-right (wide)
            0,
            floor_height,  # bottom-left (wide)
        )

        floor_pil = pil_image.resize((floor_width, floor_height), Image.LANCZOS)
        floor = floor_pil.transform(
            (floor_width, floor_height), Image.QUAD, floor_quad, Image.BICUBIC
        )
        ...
        # Instructions
        title = font.render(
            "PIL Perspective Transform Test - Press ESC to close", True, (255, 255, 255)
        )
        ...
        assert left_wall.size == (wall_width, wall_height)
        assert right_wall.size == (wall_width, wall_height)
        assert floor.size == (floor_width, floor_height)
        assert left_wall.mode == pil_image.mode
        assert right_wall.mode == pil_image.mode
        assert floor.mode == pil_image.mode
