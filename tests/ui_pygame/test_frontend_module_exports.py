...
def test_refactored_frontend_facades_preserve_primary_exports():
    ...
    assert modern_character_screen.ModernCharacterScreen.__name__ == ("ModernCharacterScreen")
...
def test_composed_frontend_classes_keep_methods_on_behavior_modules():
    assert combat_manager.GUICombatManager.start_combat.__module__.endswith(".lifecycle")
    assert combat_view.CombatView.render_combat.__module__.endswith(".sprites")
    assert dungeon_manager.DungeonManager.explore_dungeon.__module__.endswith(".exploration")
    assert modern_character_screen.ModernCharacterScreen.navigate.__module__.endswith(".equipment")
