...
def test_tileset_loading():
    ...
    print(
        "✓ Wall texture loaded: "
        + str(textures.get("wall").get_size() if textures.get("wall") else "N/A")
    )
    print(
        "✓ Floor texture loaded: "
        + str(textures.get("floor").get_size() if textures.get("floor") else "N/A")
    )
    print(
        "✓ Ceiling texture loaded: "
        + str(textures.get("ceiling").get_size() if textures.get("ceiling") else "N/A")
    )

    pygame.quit()
    print("\n✓ All textures loaded successfully!")
    print("✓ Tileset integration complete!")
    assert set(textures) == {"wall", "floor", "ceiling"}
    assert textures["wall"].get_size()[0] > 0
    assert textures["floor"].get_size()[1] > 0
    assert textures["ceiling"].get_size()[0] > 0
