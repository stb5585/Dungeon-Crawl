...
# Functions
def options(player, enemy, wmap, tile, action_list=None):
    """
    ...
    if action_list is not None:
        for action in action_list:
            print(action)
        if 'Chest' not in enemy.name and 'Boss' not in tile.intro_text(player):
            print(actions.Flee())
        action_input = input('Action: ').lower()
        return action_input


def player_turn(player, enemy, wmap, tile, available_actions, combat):
    valid_entry = False
    flee = False
    action_input = options(player, enemy, wmap, tile, available_actions)
...
def battle(player, enemy, wmap):
    ...
    while combat:
        if player.status_effects['Stun'][0]:
            ...
            while True:
                valid_entry, combat = player_turn(player, enemy, wmap, tile, available_actions, combat)
                if valid_entry:
                    break
